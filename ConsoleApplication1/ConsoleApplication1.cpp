...
float sensor_Height  = 8.466666666666665;
...
float focal_length = 26.73;//;// 26.73;
RNG rng(12345);
int coods_Left[4][4];
int coods_Right[4][4];
...
//	GaussianBlur(gray_image, gau_blur, Size(7, 7), 0, 0);
//	Canny(gau_blur, edge, 50, 150, 3);
//	cv::dilate(edge, dil, cv::Mat(), cv::Point(-1, -1));
//	cv::erode(dil, erode1, cv::Mat(), cv::Point(-1, -1));
//	
//	RNG rng(12345);
//	vector<vector<Point> > contours;
//	vector<Vec4i> hierarchy;
//	
//	//Canny(erode1, canny_output, thresh, thresh * 2, 3);
...
void thresh_callback(int, void*)
{
	...
	findContours(erode1, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE, Point(0, 0));
	Mat drawing = Mat::zeros(erode1.size(), CV_8UC3);
	for (size_t i = 0; i< contours.size(); i++)
	{
		Scalar color = Scalar(rng.uniform(0, 255), rng.uniform(0, 255), rng.uniform(0, 255));
		drawContours(drawing, contours, (int)i, color, 2, 8, hierarchy, 0, Point());
...
int main(int, char** argv)
{
	...
	const char * left_bg_name = "C:/Users/Raghunandan/Desktop/BG_LEFT.jpg";
	const char * right_bg_name = "C:/Users/Raghunandan/Desktop/BG_RIGHT.jpg";
	...
	do
	{
		if (leftOrRight == 0)
		{
			...
			Mat bg_image = imread(left_bg_name);
			printf("FOR LEFT IMAGE:\n");
		}
		else if (leftOrRight == 1)
		{
			src_mat = imread(right_file_name);
			Mat bg_image = imread(right_bg_name);
...
void calculate_Original_Dimensions()
{
	depth = 1000; //(focal_length*distance_between_Camera) / disparity;
	printf("\nDepth : %lf", depth);
	object_Height = coods_Left[2][1]-coods_Left[1][1];
	float Real_height = (depth*object_Height*sensor_Height) / (focal_length * image_Height);
	printf("\nReal_height: %lf", Real_height);
	//Real height (mm) = (Distance Of Obj(mm) * Object Height(pixels) * Sensor_Height (mm))/( focal_Length(mm) * imght(pixels));
	//Real width (mm) = (Distance Of Obj(mm) * Object Width(pixels) * Sensor_Width(mm))/( focal_Length(mm) * imgwidth(pixels));
